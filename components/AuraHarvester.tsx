...
export default function AuraHarvester({ onAnalysisComplete }: AuraHarvesterProps) {
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center gap-3 p-1.5 bg-white/5 rounded-2xl w-fit">
                <button
                    onClick={() => { setActiveTab('url'); setError(null); }}
                    className={`flex items-center gap-2 px-4 py-2 rounded-xl text-[10px] font-black tracking-widest transition-all ${activeTab === 'url' ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'text-white/30 hover:text-white/60'}`}
                >
                    <Link2 className="h-3.5 w-3.5" /> URL UPLINK
                </button>
                <button
                    onClick={() => { setActiveTab('text'); setError(null); }}
                    className={`flex items-center gap-2 px-4 py-2 rounded-xl text-[10px] font-black tracking-widest transition-all ${activeTab === 'text' ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'text-white/30 hover:text-white/60'}`}
                >
            ...
            <div className="relative">
                {activeTab === 'url' ? (
                    ...
                    <textarea
                        ...
                        rows={4}
                        className="w-full px-6 py-4 bg-white/[0.03] border border-white/10 rounded-2xl text-white placeholder-white/10 text-sm font-bold tracking-tight focus:outline-none focus:border-primary/50 focus:ring-4 focus:ring-primary/10 transition-all resize-none"
                    />
                )}
            </div>
