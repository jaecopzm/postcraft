...
export async function POST(request: NextRequest) {
  try {
    ...
    const { topic, platforms, tone, variationCount, brandGuide, isCampaignMode } = validation.data;
    ...
    const results = await Promise.all(
      platforms.map(async (platform) => {
        const variations = await generateContent(topic, platform, tone, variationCount, brandGuide, isCampaignMode);
        return {
          platform,
